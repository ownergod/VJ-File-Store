...
def formate_file_name(file_name):
    ...
    for c in chars:
        file_name.replace(c, "")
    file_name = '@mainupdatagod ' + ' '.join(filter(lambda x: not x.startswith('http') and not x.startswith('@') and not x.startswith('www.'), file_name.split()))
    return file_name
...
async def start(client, message):
    ...
    if len(message.command) != 2:
        buttons = [[
            InlineKeyboardButton('💝 𝐌𝐀𝐈𝐍 𝐂𝐇𝐇𝐀𝐍𝐀𝐋 ', url='https://t.me/mainupdatagod')
            ],[
            InlineKeyboardButton('🔍 𝐌𝐀𝐈𝐍  ', url='https://t.me/mainupdatagod'),
            InlineKeyboardButton('🤖 𝐔𝐏𝐃𝐀𝐓𝐄  ', url='https://t.me/mainupdatagod')
            ],[
